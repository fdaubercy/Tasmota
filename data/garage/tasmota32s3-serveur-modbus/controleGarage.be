...
class CONTROLE_GARAGE : Driver
    ...
    def init()
        ...
        if (controleGeneral.nbIOActivesJSON["thermometres"]["actives"]["nb"] > controleGeneral.nbIOActivesJSON["thermometres"]["reels"]["nb"])
            if (drivers["ModBus"].find("activation", "OFF") == "ON")
                try
                    # Enregistre sur telemetrie les capteurs de température virtuels ModBus
                    var thermometres = modules["garage"]["environnement"].find("thermometres", false)
                    if (thermometres)
                        # Recherche les thermometres virtuels ModBus
                        for cleTH: thermometres.keys()
                            if type(thermometres[cleTH]) != "instance"   continue    end
                            tasmota.yield()

                            if (thermometres[cleTH].find("activation", "OFF") == "ON" && thermometres[cleTH].find("virtuel", "OFF") != "OFF")
                                if (string.split(thermometres[cleTH]["virtuel"], "_")[0] == "ModBus")
                                    if (thermometres[cleTH].find("idModBus", false) != false)
                                        # Construit la trame
                                        var trameModBus = 	{
                                                                "deviceaddress": drivers["ModBus"]["environnement"]["TasmotaSlaveModBus1"]["id"], 
                                                                "functioncode": 0x04, 
                                                                "startaddress": thermometres[cleTH]["type"] + thermometres[cleTH]["idModBus"] - 1, 
                                                                "type": "float", 
                                                                "count": 1, 
                                                                "values": 0
                                                            }

                                        # Demande régulièrement l'état des thermomètres virtuels ModBus
                                        tasmota.set_timer(drivers["ModBus"]["timeoutReponse"], 
                                                            def()   
                                                                tasmota.cmd("ModBusSend " + json.dump(trameModBus), boolMute)
                                                                tasmota.add_cron("*/60 * * * * *",  /-> tasmota.cmd("ModBusSend " + json.dump(trameModBus), boolMute), "majThermo_TasmotaSlaveModBus")
                                                            end)
                                    end
                                end
                            end
                    ...
                    garageFonctions.log(string.format("INIT_GARAGE_ERREUR: %s -> %s", error, message), LOG_LEVEL_ERREUR)
                end
            end
        end
        ...
        if (controleGeneral.nbIOActivesJSON["analogiques"]["actives"]["nb"] > controleGeneral.nbIOActivesJSON["analogiques"]["reels"]["nb"])
            if (drivers["ModBus"].find("activation", "OFF") == "ON")
                try
                    # Enregistre sur telemetrie les capteurs analogiques virtuels ModBus
                    var analogiques = modules["garage"]["environnement"].find("analogiques", false)
                    if (analogiques)
                        # Recherche les capteurs analogiques virtuels ModBus
                        for cleAna: analogiques.keys()
                            ...
                            if (analogiques[cleAna].find("activation", "OFF") == "ON" && analogiques[cleAna].find("virtuel", "OFF") != "OFF")
                                if (string.split(analogiques[cleAna]["virtuel"], "_")[0] == "ModBus")
                                    if (analogiques[cleAna].find("idModBus", false) != false)
                                        # Construit la trame
                                        var trameModBus = 	{
                                                                "deviceaddress": drivers["ModBus"]["environnement"]["TasmotaSlaveModBus1"]["id"], 
                                                                "functioncode": 0x04, 
                                                                "startaddress": analogiques[cleAna]["type"] + analogiques[cleAna]["idModBus"] - 1, 
                                                                "type": "float", 
                                                                "count": 1, 
                                                                "values": 0
                                                            }
                                        ...
                                        # Demande régulièrement l'état des thermomètres virtuels ModBus
                                        tasmota.set_timer(drivers["ModBus"]["timeoutReponse"] * 2, 
                                                            def()   
                                                                tasmota.cmd("ModBusSend " + json.dump(trameModBus), boolMute)
                                                                tasmota.add_cron("*/60 * * * * *",  /-> tasmota.set_timer(drivers["ModBus"]["timeoutReponse"] * 2,
                                                                                                                            def()
                                                                                                                                tasmota.cmd("ModBusSend " + json.dump(trameModBus), boolMute)
                                                                                                                            end, "majThermo_TasmotaSlaveModBus"))
                                                            end)
                                    end
                                end
                            end
