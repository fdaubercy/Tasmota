			...
			if modules[cleModule]["activation"] == "ON"
				...
				for cleDevices: env.keys()
					if type(env[cleDevices]) != "instance"
						...
						if cleDevices == "leds"
							var ledPower = modules[cleModule].find("activation", "OFF")
							if tasmota.cmd("LedPower", boolMute)["LedPower1"] != ledPower
								...
								tasmota.cmd(string.format("Backlog LedPower %s; SetOption31 %s;", ledPower, (ledPower=="ON" ? "OFF" : "ON")), boolMute)
							end	
							tasmota.cmd(string.format("LedState %i", json["modules"]["leds"]["ledState"]), boolMute)
						end
