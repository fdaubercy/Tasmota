...
configGlobal.configGlobalByJson = def()
	import string
	...
	var template = persist.find("parametres")["template"]
		template["NAME"] = persist.find("parametres")["template"]["NAME"]
	var gpioPinUtilises = []

	var ordreGPIO = []
	if (persist.find("parametres")["diverses"].find("typeESP", "ESP32") == "ESP32")
	...
	elif (persist.find("parametres")["diverses"].find("typeESP", "ESP32") == "ESP32S3")
	...
	data = persist.find("parametres")["serveur"]["wifi"]
	...
	data = persist.find("parametres")["serveur"]["IP"]
	...
	data = persist.find("parametres")["serveur"]["mqtt"]
	...
	data = persist.find("parametres")["diverses"]["localisation"]
	...
	data = persist.find("parametres")["diverses"]["fuseauHoraire"]	
    ...
    data = persist.find("parametres")["diverses"]
    ...
    enregistrePersistant = globalFonctions.recupereTemplate(persist.find("parametres"), ordreGPIO, template, componentesInverse)
	...
	modules = persist.find("parametres")["modules"]
	if modules["activation"] == "ON"
		log ("CONTROLE_GENERAL: Paramètre les éléments non-spécifiques à certains modules !", LOG_LEVEL_DEBUG)
		for cleModule: modules.keys()
			...
			if modules[cleModule]["activation"] == "ON"				# ex: ["modules"]["cuve"]["activation"]
				...
				var env = modules[cleModule]["environnement"]			# ex: env = ["modules"]["cuve"]["activation"]["environnement"]
				for cleDevices: env.keys()								# ex: cleDevices = ["modules"]["cuve"]["activation"]["environnement"]["ADS1115"]
					...
					for cleDev: env[cleDevices].keys()
						if type(env[cleDevices][cleDev]) != "instance"	continue	end		# ex: cleDev = ["modules"]["cuve"]["activation"]["environnement"]["ADS1115"]["activation"]
						j += 1
					end

					if env[cleDevices].find("activation", "ON") == "OFF"
						# On stoppe la boucle et passe aux devices d'environnement suivantes
						break
					else log (string.format("CONFIG_GLOBAL: Parametre les %i %s du module %s !", j, cleDevices, cleModule), LOG_LEVEL_DEBUG)
					end
					...
					for cleDev: env[cleDevices].keys()
						...
						typeApp = ""
						# ex: cleDev = ["modules"]["cuve"]["activation"]["environnement"]["ADS1115"]["Ana1"]	
						...
						if env[cleDevices][cleDev].find("activation", "OFF") == "ON" && ((env[cleDevices][cleDev].find("pin", -1) != -1  && env[cleDevices][cleDev].find("virtuel", "OFF") == "OFF") || env[cleDevices][cleDev].find("virtuel", "OFF") == "ON")
	...
	if enregistrePersistant
        ...
        template["NAME"] = persist.find("parametres")["template"]["NAME"]
        persist.parametres["template"] = template

		log ("CONTROLE_GLOBAL: Modifie & Enregistre _persist.json !", LOG_LEVEL_DEBUG)
		persist.parametres = persist.parametres	
		persist.save() 
	end
