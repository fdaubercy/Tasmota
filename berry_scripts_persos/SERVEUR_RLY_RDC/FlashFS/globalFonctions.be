...
def configByJson(json)
	...
	if str(tasmota.cmd("IPAddress4", boolMute)["IPAddress4"]) != str(data["DNSServer"])
		if data["DNSServer"] != "" && data["DNSServer"] != "0.0.0.0"
	...
	# Paramètre le lecteur de Carte SD dans le modele et sur interface web en fonction des persist.json	
	# Si le lecteur de carte SD est activé
	var SD = json["diverses"].find("SD", false)
	if SD["activation"] == "ON" && SD != false
		if SD
			log ("CONTROLE_GENERAL: Parametre les pins du lecteur de carte SD !", LOG_LEVEL_DEBUG)

			for cle: SD["pins"].keys()
				typeApp = ""

				# Uniquement si pin != -1 & type != ""
				id = int(SD["pins"][cle]["type"] & 0x1F) + 1
				#typeApp = int((SD["pins"][cle]["type"] & 0xFFE0) >> 5) + id - 1
				typeApp = int((SD["pins"][cle]["type"]) >> 5) + id - 1
				pin = SD["pins"][cle]["pin"]
				...
				if pin != -1 && typeApp != ""	
					...
					if template["GPIO"][pos] != typeApp
						template["GPIO"][pos] = typeApp
						log (string.format("CONTROLE_GLOBAL: Modifie en json le type du connecteur pour carte SD: %s = %i !", cle, int(typeApp) + id - 1), LOG_LEVEL_DEBUG)
	...
	if modules["activation"] == "ON"
		for cleModule: modules.keys()
			...
			if modules[cleModule]["activation"] == "ON"
				...
				var switchs = modules[cleModule]["environnement"].find("capteurs", false)
				...
				if switchs
					...
					log (string.format("CONTROLE_GLOBAL: Il y a %i capteurs a parametrer !", modules[cleModule]["environnement"]["capteurs"].size() + (modules[cleModule]["environnement"].find("interrupteurs") != nil ? modules[cleModule]["environnement"]["interrupteurs"].size() : 0)), LOG_LEVEL_DEBUG)
					...
					for cleSwitch: switchs.keys()
						...
						if switchs[cleSwitch]["activation"] == "ON"
							...
							typeApp = int((switchs[cleSwitch]["type"] >> 5) & 0xFFE0) + id - 1
							...
							if pin != -1 && typeApp != ""	
								...
								if int(reponseCMD) != switchs[cleSwitch]["SwitchMode"]
									# Quand le circuit est fermé, Tasmota enverra ON
									log(string.format("CONTROLE_GLOBAL: Parametrage du mode du capteur ou interrupteur %i = SwitchMode %i!", id, switchs[cleSwitch]["SwitchMode"]), LOG_LEVEL_DEBUG)
									tasmota.cmd(string.format("Backlog SwitchMode%i %i;", id, switchs[cleSwitch]["SwitchMode"]), boolMute)
								end		
							end
						end
					end	
				...
				if boutons
					...
					for cleBTN: boutons.keys()
						...
						if boutons[cleBTN]["activation"] == "ON"
							...
							typeApp = int((boutons[cleBTN]["type"] >> 5) & 0xFFE0) + id - 1
							...
							if pin != -1 && typeApp != ""	
								...
								reponseCMD = tasmota.cmd(string.format("SwitchMode%i", id), boolMute)[string.format("SwitchMode%i", id)]
								if reponseCMD != boutons[cleBTN]["SwitchMode"]
									# Quand le circuit est fermé, Tasmota enverra OFF
									log(string.format("CONTROLE_GLOBAL: Parametrage du mode du bouton %i = SwitchMode %i!", id, boutons[cleBTN]["SwitchMode"]), LOG_LEVEL_DEBUG)
									tasmota.cmd(string.format("Backlog SwitchMode%i %i;", id, boutons[cleBTN]["SwitchMode"]), boolMute)
								end						
							end
						end
					end	
				...
				if thermo
					...
					for cleTH: thermo.keys()
						...
						if thermo[cleTH]["activation"] == "ON"					
							...
							typeApp = int((thermo[cleTH]["type"] >> 5) & 0xFFE0) + id - 1
							pin = thermo[cleTH]["pin"]							
				...
				# Paramètres les Capteurs Hygro/Thermo DHT22 dans le modele et sur interface web en fonction des persist.json	
				var debitmetres = modules[cleModule]["environnement"].find("debitmetres", false)

				if debitmetres
					log (string.format("CONTROLE_GENERAL: Parametre capteurs de débit du module %s !", cleModule), LOG_LEVEL_DEBUG)
					log (string.format("CONTROLE_GLOBAL: Il y a %i débitmètres a parametrer !", debitmetres.size()), LOG_LEVEL_DEBUG)
					
					for cleDebit: debitmetres.keys()
						typeApp = ""


						if debitmetres[cleDebit]["activation"] == "ON"		
							# Uniquement si pin != -1 & type != ""
							typeApp = debitmetres[cleDebit]["type"]
							pin = debitmetres[cleDebit]["pin"]		

							# Uniquement si pin != -1 & type != ""
							id = int(debitmetres[cleDebit]["type"] & 0x1F) + 1
							typeApp = int((debitmetres[cleDebit]["type"] >> 5) & 0xFFE0) + id - 1
							pin = debitmetres[cleDebit]["pin"]	
							...
							if pin != -1 && typeApp != ""	
								...
								if template["GPIO"][pos] != typeApp
									template["GPIO"][pos] = typeApp
									log (string.format("CONTROLE_GLOBAL: Modifie en json le type Débitmètre = %i !", int(typeApp)), LOG_LEVEL_DEBUG)
